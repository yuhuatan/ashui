 ...
 ### TASK [install python-xml for opensuse only if python2 is installed already] *** skipping
 ### TASK [gather facts] *** skipping
 ### TASK [gather and delegate facts] *** ok
 ## tasks
 ### import_role: ceph-default
 导入ceph默认变量
 ### TASK [create filtered clients group] ***
 ### import_role: ceph-facts
 ...
 ##### TASK [ceph-facts : check if it is atomic host] *** ok
 检查是否是原子主机系统，原子系统不是传统通用的操作系统，原子系统主要设计来运行容器。
 ##### TASK [ceph-facts : set_fact is_atomic] *** ok
 获取原子主机的检查状态赋值给注册变量is_atomic
 ##### import_tasks: container_binary.yml
 设置容器管理命令
 ###### TASK [ceph-facts : check if podman binary is present] *** ok
 检查podman是否存在
 ###### TASK [ceph-facts : set_fact container_binary] *** ok
 根据条件设置容器管理程序,这里为podman
 ##### TASK [ceph-facts : set_fact ceph_cmd] *** ok
 根据条件设置ceph启动命令，如果是容器方式部署就用podman启动，否则为ceph，这里是ceph
 ##### TASK [ceph-facts : set_fact discovered_interpreter_python] *** sipping
 ##### TASK [ceph-facts : set_fact discovered_interpreter_python if not previously set] *** ok
 ##### TASK [ceph-facts : set_fact ceph_release ceph_stable_release] *** ok
 设置ceph版本号
 ##### TASK [ceph-facts : set_fact monitor_name ansible_facts['hostname']] *** ok
 设置各monitor节点的monitor_name
 ##### block [find a running monitor]
 ###### TASK [ceph-facts : set_fact container_exec_cmd] *** skipping
 ###### TASK [ceph-facts : find a running mon container] *** skipping
 ###### TASK [ceph-facts : check for a ceph mon socket] *** ok
 ###### TASK [ceph-facts : check if the ceph mon socket is in-use] *** skipping
 ###### TASK [ceph-facts : set_fact running_mon - non_container] *** skipping
 ###### TASK [ceph-facts : set_fact running_mon - container] *** skipping
 ###### TASK [ceph-facts : set_fact _container_exec_cmd] *** skipping
 ###### TASK [ceph-facts : get current fsid if cluster is already running] *** ok
 ok: [cepht1 -> cepht1]
 ##### TASK [ceph-facts : set_fact current_fsid rc 1] *** skipping
 ##### TASK [ceph-facts : get current fsid] *** skipping
 ##### TASK [ceph-facts : set_fact fsid] *** skipping
 ##### TASK [ceph-facts : set_fact fsid from current_fsid] *** skipping
 skipping: [cepht1]
 ##### block: [fsid related tasks]
 ###### TASK [ceph-facts : generate cluster fsid] *** changed
 changed: [cepht1 -> cepht1]
 ###### TASK [ceph-facts : set_fact fsid] *** ok
 ##### import_tasks: devices.yml
 ###### TASK [ceph-facts : resolve device link(s)] *** skipping
 devices有定义同时osd_auto_discovery为false时找出符号链接后面的真正磁盘文件
 ###### TASK [ceph-facts : set_fact build devices from resolved symlinks] *** skipping
 devices有定义同时osd_auto_discovery为false时根据上个task的返回设置磁盘盘符。
 ###### TASK [ceph-facts : set_fact build final devices list] *** skipping
 devices有定义同时osd_auto_discovery为false时根据上个task的返回设置磁盘序列。
 ###### TASK [ceph-facts : resolve dedicated_device link(s)] *** skipping
 ###### TASK [ceph-facts : set_fact build dedicated_devices from resolved symlinks] *** skipping
 ###### TASK [ceph-facts : set_fact build final dedicated_devices list] *** skipping
 ###### TASK [ceph-facts : resolve bluestore_wal_device link(s)] *** skipping
 ###### TASK [ceph-facts : set_fact build bluestore_wal_devices from resolved symlinks] *** skipping
 ###### TASK [ceph-facts : set_fact build final bluestore_wal_devices list] *** skipping
 ###### TASK [ceph-facts : set_fact devices generate device list when osd_auto_discovery] *** skipping
 ##### block: [backward compatibility tasks related] *** skipping
 ###### TASK [ceph-facts : get ceph current status] *** skipping
 skipping: [cepht1]
 ...
 ok: [cepht2]
 skipping: [cepht1,cepht3,cepht4,cepht5]
 ##### TASK [ceph-facts : set_fact rgw_yh_esync_instances] ***
 ok: [cepht3]
 skipping: [cepht1,cepht2,cepht4,cepht5]
 ...
 changed: [cepht1,cepht2,cepht3,cepht4,cepht5]
